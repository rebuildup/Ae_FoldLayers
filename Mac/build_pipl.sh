...
PIPL_SOURCE="$PLUGIN_DIR/FoldLayers_PiPL.r"
PIPL_OUTPUT="$PLUGIN_PATH/Contents/Resources/FoldLayers.rsrc"

echo "=== PiPL Resource Generation (AEGP) ==="
...
for required_file in "AEConfig.h"; do
...
if echo "$PIPL_STRINGS" | grep -q "EntryPointFunc"; then
  FOUND_ENTRY_POINT=true
  echo "  ✓ Found 'EntryPointFunc' entry point"
fi
...
if [ "$FOUND_ENTRY_POINT" = false ]; then
  echo "::error::EntryPointFunc not found in PiPL resource!"
  exit 1
fi
